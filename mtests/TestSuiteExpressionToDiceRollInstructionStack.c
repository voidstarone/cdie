...
void test_dice_roll_instruction_stack_from_expression() {
...
int test_suite_expression_to_dice_roll_instruction_stack(int(*init_suite)(void), int(*clean_suite)(void) ) {
	...
	if (NULL == CU_add_test(pSuite, "test_dice_roll_instruction_stack_from_expression", 
        test_dice_roll_instruction_stack_from_expression)) {
        CU_cleanup_registry();
        return CU_get_error();
    }
